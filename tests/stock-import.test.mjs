...
const entry = fs.readFileSync('stock_summary.html', 'utf8');
const page = fs.readFileSync('src/stock-summary/StockSummaryPage.tsx', 'utf8');
const domain = fs.readFileSync('src/domain/stock-summary.ts', 'utf8');
const repository = fs.readFileSync('src/services/stock-summary-repository.ts', 'utf8');
const controller = fs.readFileSync('src/stock-summary/StockImportController.ts', 'utf8');
const html = [entry, page, domain, repository, controller].join('\n');
...
assert.match(html, /STOCK_SUMMARY_PAGE_SIZE = 1000/, '库存读取应按1000条分页');
assert.match(html, /async function fetchAllRows/, '应提供完整分页读取函数');
assert.match(html, /\.range\(from, from \+ STOCK_SUMMARY_PAGE_SIZE - 1\)/, '每页应使用Supabase range读取');
assert.match(html, /fetchAllRows<StockRecord>\(/, '库存表应通过分页读取');
...
assert.match(html, /exportEmployeeStocks/, '库存页应保留导出功能');
assert.match(entry, /src="\/src\/stock-summary\/main\.tsx"/, '主入口应由 React + TypeScript 接管');
