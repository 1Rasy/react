...
export type CtDealerOutboundImportControllerOptions = {
...
export type CtDealerOutboundRunOptions = {
  file: Pick<File, 'arrayBuffer'>;
  setStatus: (text: string, error?: boolean) => void;
};

export function createCtDealerOutboundImportController(options: CtDealerOutboundImportControllerOptions) {
  const { repository, xlsx, alert, now = Date.now } = options;

  return {
    async importFile({ file, setStatus }: CtDealerOutboundRunOptions): Promise<void> {
      ...
      const result = buildCtDealerOutboundRows(rows, whitelist, now());
      if (!result.payload.length) throw new Error(ctDealerOutboundZeroRowsError(result));
